...
`cli dispatch` never talks to Docker directly for image/container lifecycle —
...
~/www/agent-sandbox/install.sh   # symlinks ag-sbx onto your PATH
```
...
not [opencode](https://opencode.ai) (the execution backend `cli dispatch`
...
`cli` exports `AG_SBX_IMAGE` from that value before every `ag-sbx` call, so
...
(`opencode.db`) that must never be mounted or copied wholesale, so `cli`
...
`cli` copies whatever's in `~/.local/share/opencode/auth.json`.
